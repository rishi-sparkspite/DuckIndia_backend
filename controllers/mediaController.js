...
const unrestricted_upload = async (user, file, group = null, IsPublic = true) => {
  try {
    const userHash = helpers.hashUID(user._id);
    const time = Date.now().toString();
    const fileName = `${helpers.generateUniqueId()}${time}${path.extname(file.originalname)}`;
    const filePath = `public/unrestrictedAccess/${userHash}/${fileName}`;
...
const upload = async (user, file, group = null, IsPublic = true) => {
  try {
    const userHash = helpers.hashUID(user._id);
    const time = Date.now().toString();
    const fileName = `${helpers.generateUniqueId()}${time}${path.extname(file.originalname)}`;
    ...
    const filePath = `media/${userHash}/${fileName}`;
...
const fetch = async (user, name) => {
  const media = await MediaModel.findOne({ name: name }).lean();
  if (!media) {
    return { "success": false, "error": "File not found!" };
  }

  console.log("1 ", media.owner == user, " 2", media.public);
  if (media.owner == user || media.public) {
    const bucket = admin.storage().bucket(process.env.STORAGE_BUCKET);
    ...
    return { "success": false, "error": "You are not authorized to view this file!" };
  }
};
